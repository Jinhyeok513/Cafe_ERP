...
FastAPI retains its existing `/api/*` routes, so its deployed health endpoint is `/backend/api/health`. Vercel supplies `BACKEND_URL` to the Next.js service for server-to-server requests. Local development continues to use `CAFE_API_URL`, which takes precedence when provided.
